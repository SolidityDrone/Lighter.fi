...
contract Swapper {
    ...
    string public  integrator = "lifi-staging-api";
    ...
    *
    * This function unpacks the provided response data using the public `unpackBytes` function.
    * It constructs the path array for the swap operation, including tokenFrom, middleTokens, and tokenTo.
    * The calldata for the swap is then generated using the public `ConcatCalldata` function.
    * Finally, the swap operation is initiated using the public `swap` function, passing relevant parameters.
    */
    ...
    */
    function unpackBytes(bytes memory data) public pure returns (bytes32[2] memory, bytes16[1] memory, address[] memory, bytes4[1] memory) {
    ...
    */
    function uintToBytes32(uint256 u256) public pure returns (bytes32) {
    ...
    * @return bytes32 representation of the input uint256 value.
    */
    function bytes32tobytes4(bytes32 b32) public pure returns(bytes4){
        return (bytes4(b32));
    }

    function conversion(bytes memory data) public pure returns (bytes memory){
        string memory resultString = bytesToString(data);
        return hexStringToBytes(resultString);
    }
    ...
    require(bytes(hexString).length > 0, "Input string is empty");

    // Ensure the input string has an even length
    require(bytes(hexString).length % 2 == 0, "Invalid hex string length");
    ...
    for (uint i = 0; i < bytes(hexString).length; i += 2) {
        uint8 high = hexCharToBytes1(hexString, i) * 16;
        uint8 low = hexCharToBytes1(hexString, i + 1);
        result[i / 2] = bytes1(uint8(high + low)); // Use bytes1 instead of byte
    }

    return result;
}

    function hexCharToBytes1(string memory hexString, uint index) internal pure returns (uint8) {
        bytes1 char = bytes1(bytes(hexString)[index]);
